...
private extension HomeView {
    ...
    var riskScore: Int {
        guard let today = todayVital, let hr = heartRateStats, let br = breathingStats else { return 8 }
    ...
    var overviewBlurb: String {
        // Keep the exact layout, but update copy.
        if todayVital == nil {
            return "Run today’s video test\nto generate insights\nfrom your past data."
        }
    ...
    var heartRateCardText: String {
        ...
        let stableLine = stabilityLine(current: todayVital.map { Double($0.heartRate) }, stats: hr, unit: "bpm")
        return "Average: \(Int(hr.avg.rounded())) bpm\nNormal: \(Int(hr.low.rounded()))–\(Int(hr.high.rounded())) bpm\n\(stableLine)"
    }
    ...
    var breathingCardText: String {
        ...
        let stableLine = stabilityLine(current: todayVital.map { Double($0.breathingRate) }, stats: br, unit: "rpm")
        return "Average: \(Int(br.avg.rounded())) rpm\nNormal: \(Int(br.low.rounded()))–\(Int(br.high.rounded())) rpm\n\(stableLine)"
    }
    ...
    var suggestionLargeDescription: String {
        if let today = todayVital, let hr = heartRateStats, Double(today.heartRate) > hr.high {
        ...
        if let today = todayVital, let br = breathingStats, Double(today.breathingRate) > br.high {
    ...
    func stabilityLine(current: Double?, stats: Stats, unit: String) -> String {
        guard let current else {
            return "Based on your past data."
        }
        let within = (current >= stats.low && current <= stats.high)
        let stable = stats.stdDev <= (unit == "bpm" ? 8 : 2)
        ...
        return "A bit outside your normal range."
    }
